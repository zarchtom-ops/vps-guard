...
- SSH 失败登录日志统计、攻击 IP 排名和人工封禁
- 网络暴露面检查：读取本机监听 socket，识别公网绑定端口，不执行主动探测
- 普通用户列表与账号锁定
- 所有面板变更的本机审计日志
...
“网络暴露面”页面通过系统自带的 `ss -lntu` 读取监听 socket，标记绑定到 `0.0.0.0`、`::` 或非回环地址的端口，并显示风险等级。检查只读本机状态，不扫描互联网、不发送探测流量、不上传端口信息。它适合在变更防火墙或部署新服务后快速复核：面板本身默认绑定回环地址，公网监听端口应当有明确业务理由并配套防火墙规则。

### IP 爆破检测

## 四点一、IP 爆破检测

打开面板中的“爆破检测”，系统会读取最近 24 小时的 SSH 认证失败记录，并展示失败总数、攻击来源数量、尝试用户名、最近出现时间和风险等级。

日志读取顺序为：

1. `/var/log/auth.log`（Ubuntu、Debian）
2. `/var/log/secure`（RHEL、Rocky、AlmaLinux）
3. `journalctl` 中的 `ssh` / `sshd` 服务日志

检测是只读的，不会因为统计结果自动封禁 IP。确认某个地址为攻击来源后，可以在列表中点击“封禁 IP”，面板会要求二次确认，并调用 Fail2Ban 的 `sshd` 监狱执行封禁。若服务器未安装或未运行 Fail2Ban，封禁操作会返回明确错误。

日志只在服务器本地处理，面板不会上传认证日志内容。
